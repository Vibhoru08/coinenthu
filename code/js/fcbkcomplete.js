...
(function( $, undefined ) {
  $.fn.fcbkcomplete = function(opt) {
    return this.queue( function() {
      ...
      function createFCBK() {
        holder = $('<ul class="holder form-control"></ul>').width('auto');
        if (options.attachto) {
        ...
        complete = $('<div class="facebook-auto">').width('100%');
        ...
        //feed = $('<ul id="'+elemid+'_feed" class="user_auto_complate_w"></ul>').width('60%');
        feed = $('<ul id="'+elemid+'_feed" class="user_auto_complate_w"></ul>');
      ...
      function addItem(title, value, preadded, locked, focusme) {
        ...
        clear_feed(1);
		$('#currentRecipientId').val(id);
		//addRecipientCompany(xssDisplay(value, 1));
        return id;
      }
      ...
      function addMembers(etext, data) {
		...
		if(typeof etext != 'undefined'){
			...
			$.each(cache.search('*'), function (i, object) {
			  if (options.filter_selected && element.children('option[value="' + object.key + '"]').hasClass("selected")) {
				//nothing here...
			  } else {
				content += '<li rel="' + object.key + '">' + object.value + '</li>';
				counter++;
				maximum--;
			  }
			});
